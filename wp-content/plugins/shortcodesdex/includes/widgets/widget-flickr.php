...
class stag_flickr_widget extends WP_Widget{
	...
	function widget( $args, $instance ) {
		...
		//echo 'http://api.flickr.com/services/feeds/photos_public.gne?ids='.$flickr_id.'&lang=en-us&format=rss_200';
		if( !is_wp_error( $rss ) ){
		...
		<div class="photo-gallery clearfix">
			...
			foreach( $items as $item ) {
				...
				foreach( $image_attrs as $image ) {
					...
					$width = $image['width'];
					$height = $image['height'];
					echo '<div class="item"><a target="_blank" href="' . $item->get_permalink() . '"><img src="'. $url .'" width="' . $width . '" height="' . $height . '" alt="'. $item->get_title() .'"></a></div>';
				}
			}
			}
