<?php
/**
 * Plugin Name: ShortcodesDex
 * Plugin URI: http://themeforest.net/user/dexthemes
 * Description: Shortcodes plugin for Anchro theme.
 * Version: 2.2.0
 * Author: Dexthemes
...
class StagTools {
	...
	*/
	...
	public function __construct() {
		...
		$this->includes();
		$this->my_custom_type();
		$this->my_plugin_override();
	}

	function my_plugin_override() {
		// your code here	
	include_once( 'includes/vp_maps.php' );
...
function genres_taxonomy() {
    register_taxonomy( 'portfolio-category', 'portfolio', array ('hierarchical' => true, 'label' => ('Portfolio Categories')));  // portfolio categories
	//add_post_type_support( 'portfolio', 'post-formats', array( 'aside', 'gallery' ) );
...
function myplugin_scripts() {
    wp_enqueue_script('generosity_script1_p', plugin_dir_url( __FILE__ ).'assets/js/tweetie.min.js', array(), '', true);
}
...
add_action( 'wp_enqueue_scripts', 'generosity_scripts_p' );
function generosity_scripts_p(){
wp_localize_script( 'jquery', 'generosity_shortcode_p_dir', plugin_dir_url( __FILE__ ) );
}
