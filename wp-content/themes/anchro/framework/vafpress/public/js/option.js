;(function($) {
	...
	$('.vp-js-option-form').bind('submit', function(e) {
		...
		var $loading = $('.vp-js-save-loader'),
			...
			data = {
				action: 'vafpress_ajax_' + vafpress_opt.name + '_save',
				option: option,
				nonce : vafpress_opt.nonce
			};
	...
	$('.vp-js-restore').bind('click', function(e) {
		...
		var $button = $(this),
		    ...
		    data    = {action: 'vafpress_ajax_' + vafpress_opt.name + '_restore', nonce : vafpress_opt.nonce};
		...
		$.post(ajaxurl, data, function(response) {
			...
			{
				case parseInt(vafpress_opt.SAVE_SUCCESS):
					$status.html(vafpress_opt.util_msg.restore_success);
					break;
				case parseInt(vafpress_opt.SAVE_NOCHANGES):
					$status.html(vafpress_opt.util_msg.restore_nochanges);	
					break;
				case parseInt(vafpress_opt.SAVE_FAILED):
					$status.html(vafpress_opt.util_msg.restore_failed + ': ' + response.message);
					break;
			}
			$status.fadeIn(100);
			setTimeout(function() {
				$status.fadeOut(1000, function() {
					$button.removeAttr('disabled');
					$status.fadeOut(500);
					if (response.code == parseInt(vafpress_opt.SAVE_SUCCESS))
						location.reload();
				});
			}, 2000);
		}, 'JSON');
	...
	$('#vp-js-import').bind('click', function(e) {
		...
		var $textarea      = $('#vp-js-import_text'),
		    ...
		    $button        = $(this),
		    data           = {action: 'vafpress_ajax_' + vafpress_opt.name + '_import_option', option: $textarea.val(), nonce : vafpress_opt.nonce};
		...
		$.post(ajaxurl, data, function(response) {
			...
			if (response.status)
			{
				$import_status.html(vafpress_opt.util_msg.import_success);
			}
			else
			{
				$import_status.html(vafpress_opt.util_msg.import_failed + ': ' + response.message);
			}
	...
	$('#vp-js-export').bind('click', function(e) {
		...
		var $export_status = $('#vp-js-export-status'),
		    ...
		    $button        = $(this),
		    data           = {action: 'vafpress_ajax_' + vafpress_opt.name + '_export_option', nonce : vafpress_opt.nonce};
		...
		$.post(ajaxurl, data, function(response) {
			...
			if (!$.isEmptyObject(response.option) && response.status)
			{
				...
				$export_status.html(vafpress_opt.util_msg.export_success);
			}
			else
			{
				$export_status.html(vafpress_opt.util_msg.export_failed + ': ' + response.message);
			}
