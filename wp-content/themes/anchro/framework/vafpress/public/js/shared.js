...
// hidding plugin
jQuery.fn.vafpress_slideUp = function(callback){
	var $this = this;
...
jQuery.fn.vafpress_slideDown = function(callback){
	var $this = this;
...
jQuery.fn.vafpress_fadeOut = function(callback){
	var $this = this;
...
jQuery.fn.vafpress_fadeIn = function(callback){
	var $this = this;
	if($this.hasClass('vp-hide'))
	{
...
jQuery.fn.vafpress_toggle = function(callback){
	var $this = this;
	if($this.hasClass('vp-hide'))
	{
...
vp.validateAlphabet = function(type, val) {
	...
	if (val === '' || jQuery.isArray(val) || jQuery.inArray(type, vafpress_wp.alphabet_validatable) == -1) { return true; }
...
vp.validateAlphaNumeric = function(type, val) {
	// ignore array and empty string, since they should be handled by 'required' rule
	if (val === '' || jQuery.isArray(val) || jQuery.inArray(type, vafpress_wp.alphanumeric_validatable) == -1) { return true; }
...
vp.validateNumeric = function(type, val) {
	// ignore array and empty string, since they should be handled by 'required' rule
	if (val === '' || jQuery.isArray(val) || jQuery.inArray(type, vafpress_wp.numeric_validatable) == -1) { return true; }
...
vp.validateEmail = function(type, val) {
	// ignore array and empty string, since they should be handled by 'required' rule
	if (val === '' || jQuery.isArray(val) || jQuery.inArray(type, vafpress_wp.email_validatable) == -1) { return true; }
...
vp.validateURL = function(type, val) {
	// ignore array and empty string, since they should be handled by 'required' rule
	if (val === '' || jQuery.isArray(val) || jQuery.inArray(type, vafpress_wp.url_validatable) == -1) { return true; }
...
vp.validateMaxLength = function(type, val, n) {
	// ignore array
	if (jQuery.inArray(type, vafpress_wp.maxlength_validatable) != -1) { return true; }
...
vp.validateMinLength = function(type, val, n) {
	// ignore array
	if (jQuery.inArray(type, vafpress_wp.minlength_validatable) != -1) { return true; }
...
vp.fields_validation_loop = function(fields){
	...
	for (var i = 0; i < fields.length; i++)
	{
		...
		for (var j = 0; j < rules.length; j++)
		{
			...
			{
				case 'alphabet':
					if (!vp.validateAlphabet(type, val)) { res = vafpress_wp.val_msg.alphabet.format(); }
					break;
				case 'alphanumeric':
					if (!vp.validateAlphaNumeric(type, val)) { res = vafpress_wp.val_msg.alphanumeric.format(); }
					break;
				case 'numeric':
					if (!vp.validateNumeric(type, val)) { res = vafpress_wp.val_msg.numeric.format(); }
					break;
				case 'email':
					if (!vp.validateEmail(type, val)) { res = vafpress_wp.val_msg.email.format(); }
					break;
				case 'url':
					if (!vp.validateURL(type, val)) { res = vafpress_wp.val_msg.url.format(); }
					break;
				case 'maxlength':
					n = rule.substring(q1 + 1, q2);
					if (!vp.validateMaxLength(type, val, n)) { res = vafpress_wp.val_msg.maxlength.format(n); }
					break;
				case 'minlength':
					n = rule.substring(q1 + 1, q2);
					if (!vp.validateMinLength(type, val, n)) { res= vafpress_wp.val_msg.minlength.format(n); }
					break;
				case 'maxselected':
					n = rule.substring(q1 + 1, q2);
					if (!vp.validateMaxLength(type, val, n)) { res = vafpress_wp.val_msg.maxselected.format(n); }
					break;
				case 'minselected':
					n = rule.substring(q1 + 1, q2);
					if (!vp.validateMinLength(type, val, n)) { res= vafpress_wp.val_msg.minselected.format(n); }
					break;
				case 'required':
					if (!vp.validateRequired(type, val)) { res = vafpress_wp.val_msg.required.format(); }
					break;
			}
...
vp.binding_action =	function(ids, field, func, thecase) {
	...
	if (field.type == 'vp-html') {
		$loader.vafpress_fadeIn();
	} else {
		$input.vafpress_fadeOut(function(){
			$loader.vafpress_fadeIn();
		});
	}

	jQuery.post(ajaxurl, data, function(response) {
		$loader.vafpress_fadeOut(function(){
			$input.vafpress_fadeIn();
		});
		if (response.status)
		{
			var data;
	...
	jQuery.post(ajaxurl, data, function(response) {
		...
		if (response.status)
		{
			if(response.data)
			{
				...
				$source_tr.vafpress_fadeIn();
			}
			else
			{
				$source_tr.addClass('vp-dep-inactive');
				$source_tr.vafpress_fadeOut();
			}
		}
	}, 'JSON');
};
...
if ( vafpress_wp.use_new_media_upload )
{
...
function get_url_info(html)
{
	...
	if ($el.prop('tagName') == 'A')
	{
		...
		if(type !== 'image')
		{
			iconurl = vafpress_wp.wp_include_url + 'images/crystal/' + type + '.png' ;
		}
	}
...
vp.init_fontawesome_chooser = function($elements)
{
	if (jQuery.fn.select2)
	{
		if($elements.length <= 0)
			return;
		var format = function vafpress_fontawesome_chooser_format(icon){
		...
		$elements.select2({
			...
			allowClear: true,
			placeholder: vafpress_wp.ctrl_msg.fac_placeholder
		});
	}
};
...
vp.init_select2 = function($elements)
{
	if (jQuery.fn.select2)
	{
		if($elements.length <= 0)
			return;
		$elements.select2({allowClear: true, placeholder: vafpress_wp.ctrl_msg.select2_placeholder});
	}
};
