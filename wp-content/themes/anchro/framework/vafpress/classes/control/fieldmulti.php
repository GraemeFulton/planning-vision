 ...
 */
abstract class VAFPRESS_Control_FieldMulti extends VAFPRESS_Control_Field
{
	 ...
	 * @return VAFPRESS_Control_FieldMulti Field object
	 */
	protected function _basic_make($arr)
	{
		...
		if (!empty($arr['items']))
		{
			...
			{
				$the_item = new VAFPRESS_Control_Field_Item_Generic();
		...
		if (isset($arr['default']))
		{
			...
			if(!VAFPRESS_Util_Reflection::is_multiselectable($this))
			{
		...
		foreach ($this->_raw_default as $def)
		{
			switch ($def)
			{
				case '{{all}}':
					...
					break;
				case '{{first}}':
					$first = VAFPRESS_Util_Array::first($items);
					if(!is_null($first))
	...
	public function set_default($_default) {
		if(is_array($_default) and !VAFPRESS_Util_Reflection::is_multiselectable($this))
			$_default = VAFPRESS_Util_Array::first($_default);
	...
	/**
	 * Add single item
	 * @param VAFPRESS_Control_Field_Item_ $opt Single item item
	 */
