...
class VAFPRESS_Util_Config
{
	...
	public function load($config_name, $key = '')
	{
		...
		{
			if(is_file(VAFPRESS_CONFIG_DIR . '/'. $config_name . '.php'))
			{
				$config     = require VAFPRESS_CONFIG_DIR . '/'. $config_name . '.php';//this contant uses get_template_directory 
			}
			else
			{
