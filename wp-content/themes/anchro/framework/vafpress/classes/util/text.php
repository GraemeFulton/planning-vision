...
class VAFPRESS_Util_Text
{
	...
	public static function parse_md($text)
	{
		...
		{
			//$path = VAFPRESS_FileSystem::instance()->resolve_path('includes', 'parsedown');
			$path = get_template_directory().'/framework/vafpress/includes/parsedown.php';
			require $path;//this var uses get_template_directory
		}
	...
	{
		if( empty($string) )
			echo wp_kses_normalize_entities($default);
		else
			echo wp_kses_normalize_entities($string);
	}
	...
	{
		array_walk( $array, 'VAFPRESS_Util_Text::prefix', $prefix);
		return $array;
	}
