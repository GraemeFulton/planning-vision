...
class VAFPRESS_WP_Loader
{
	...
	{
		$this->_dependencies = apply_filters( 'vafpress_dependencies_array', VAFPRESS_Util_Config::instance()->load('dependencies') );
		$this->_types        = array(
			'option'             => array(),
			'metabox'            => array(),
	...
	public function build()
	{
		...
		foreach ($styles as $name => $style) 
		{
			if(in_array($name, $req_styles) and ! wp_style_is($name, 'registered'))
				wp_enqueue_style($name, $style['path'], $style['deps']);
		}
		...
		foreach ($this->_js_data as $name => $js)
		{
			...
			wp_enqueue_script($name, $js['path'], $deps, '', true);
		...
		foreach ($this->_css_data as $name => $css)
		{
			...
			wp_enqueue_style($name, $css['path'], $req_styles);
			
		}

		do_action( 'vafpress_after_dependencies_loader_build' );
	...
	{
		$messages = VAFPRESS_Util_Config::instance()->load('messages');
		$localize = array(
			'use_upload'               => $this->_use_media_upload,
			'use_new_media_upload'     => $this->_use_wp_35_media_upload,
			'public_url'               => VAFPRESS_PUBLIC_URL,
			...
			// validatable data
			'alphabet_validatable'     => apply_filters( 'vafpress_alphabet_validatable'    , array( 'vp-textbox', 'vp-textarea' ) ),
			'alphanumeric_validatable' => apply_filters( 'vafpress_alphanumeric_validatable', array( 'vp-textbox', 'vp-textarea' ) ),
			'numeric_validatable'      => apply_filters( 'vafpress_numeric_validatable'     , array( 'vp-textbox', 'vp-textarea' ) ),
			'email_validatable'        => apply_filters( 'vafpress_email_validatable'       , array( 'vp-textbox', 'vp-textarea' ) ),
			'url_validatable'          => apply_filters( 'vafpress_url_validatable'         , array( 'vp-textbox', 'vp-textarea' ) ),
			'maxlength_validatable'    => apply_filters( 'vafpress_maxlength_validatable'   , array( 'vp-toggle', 'vp-radiobutton', 'vp-radioimage', 'vp-select' ) ),
			'minlength_validatable'    => apply_filters( 'vafpress_minlength_validatable'   , array( 'vp-toggle', 'vp-radiobutton', 'vp-radioimage', 'vp-select' ) ),
		);
	...
	private function js_unit_register($name, $extra_deps = null)
	{
		...
		if( isset($scripts[$name]) )
		{
			...
			if( !$registered or ($is_older and $override) )
			{
				...
				wp_enqueue_script($name, $script['path'], $script['deps'], $script['ver'], true);
	...
	private function css_unit_register($name, $extra_deps = null)
	{
		...
		if( isset($styles[$name]) )
		{
			...
			wp_enqueue_style($name, $style['path'], $style['deps'], isset($style['ver']) ? $style['ver'] : false);
		}
	}
