...
class VAFPRESS_Option
{
	...
	public function __construct(array $configs)
	{
		...
		$configs = array_merge(array(
			...
			'layout'                => 'fixed',
			'page_title'            => esc_html__( 'Vafpress Options', 'anchro' ),
			'menu_label'            => esc_html__( 'Vafpress Options', 'anchro' ),
			'priority'              => 10,
		), $configs);

		// options config filter
		$configs = apply_filters('vafpress_option_configuration_array'  , $configs, $configs['option_key']);
		$configs = apply_filters('vafpress_option_configuration_array-' . $configs['option_key'], $configs);
		...
		else throw new Exception(esc_html__( 'Option Key is required', 'anchro' ), 1);
		if(isset($template)) $this->set_template($template);
		else throw new Exception(esc_html__( 'Template Array/File is required', 'anchro' ), 1);
		if(isset($page_slug)) $this->set_page_slug($page_slug);
		else throw new Exception(esc_html__( 'Page Slug is required', 'anchro' ), 1);
		...
		add_action('vafpress_option_first_activation', array($this, 'initial_db_setup'));

		// check if option key not existed init data from default values
		$options = get_option( $this->get_option_key() );
		if( $options === FALSE )
		{
			do_action('vafpress_option_first_activation');
		}
		...
		// setup ajax
		add_action('wp_ajax_vafpress_ajax_' . $this->get_option_key() . '_export_option', array($this, 'ajax_export_option'));
		add_action('wp_ajax_vafpress_ajax_' . $this->get_option_key() . '_import_option', array($this, 'ajax_import_option'));
		add_action('wp_ajax_vafpress_ajax_' . $this->get_option_key() . '_save'         , array($this, 'ajax_save'));
		add_action('wp_ajax_vafpress_ajax_' . $this->get_option_key() . '_restore'      , array($this, 'ajax_restore'));
	...
	{
		VAFPRESS_WP_Util::admin_notice(esc_html__("Development Mode is Active, options' values won't be saved into database.", 'anchro'), false);
	}

	public function enqueue_scripts_and_styles()
	{	
		$opt_loader = VAFPRESS_WP_Loader::instance();
		...
		$opt_loader->add_js_data( 'vp-option', 'custom_local.SAVE_SUCCESS', VAFPRESS_Option_Control_Set::SAVE_SUCCESS );
		$opt_loader->add_js_data( 'vp-option', 'custom_local.SAVE_NOCHANGES', VAFPRESS_Option_Control_Set::SAVE_NOCHANGES );
		$opt_loader->add_js_data( 'vp-option', 'custom_local.SAVE_FAILED', VAFPRESS_Option_Control_Set::SAVE_FAILED );
	}
	...
	function save_and_reinit()
	{
		...
		do_action('vafpress_option_save_and_reinit', $opt, $result['status'], $this->get_option_key());

		// option key specific save and re-init action hook
		do_action('vafpress_option_save_and_reinit-' . $this->get_option_key(), $opt, $result['status']);
	...
	function ajax_save()
	{
		$result = $this->vafpress_verify_nonce();
		...
		if($result['status'])
		{
			...
			$option  = VAFPRESS_Util_Array::unite( $option, 'name', 'value' );
			...
			do_action('vafpress_option_before_ajax_save', $opt);

			// save and re-init options
			$result = $this->save_and_reinit();

			// after ajax save action hook
			do_action('vafpress_option_after_ajax_save', $opt, $old_opt, $result['status'], $this->get_option_key());

			// option key specific after ajax save action hook
			do_action('vafpress_option_after_ajax_save-' . $this->get_option_key(), $opt, $old_opt, $result['status']);
		}
	...
	function ajax_restore()
	{
		...
		if( $result['status'] )
		{
			...
			// before ajax save action hook
			do_action('vafpress_option_before_ajax_restore', $options);
			...
			do_action('vafpress_option_after_ajax_restore', $options, $old_opt, $result['status'], $this->get_option_key());

			// after ajax restore action hook
			do_action('vafpress_option_after_ajax_restore-' . $this->get_option_key(), $options, $old_opt, $result['status']);
		}
	...
	{
		global $vafpress_set, $vafpress_config;
		...
		if($result['status'])
		{
			...
			{
				$result['status']  = false;
				$result['message'] = esc_html__("Can not be empty", 'anchro');
			}
			else
			{
				...
				{
					$result['status']  = false;
					$result['message'] = esc_html__("Invalid data", 'anchro');
				}
			}
		}

		// after ajax import action hook
		do_action('vafpress_option_after_ajax_import', $options, $old_opt, $result['status'], $this->get_option_key());

		// after ajax import action hook
		do_action('vafpress_option_after_ajax_import-' . $this->get_option_key(), $options, $old_opt, $result['status']);
	...
	function ajax_export_option()
	{
		...
		if($result['status'])
		{
			...
			$result = array(
				'status' => true,
				'message'=> esc_html__("Successful", 'anchro'),
				'option' => $sr_options,
			);
		}

		// after ajax export action hook
		do_action('vafpress_option_after_ajax_export', $db_options, $sr_options, $result['status'], $this->get_option_key());

		// after ajax export action hook
		do_action('vafpress_option_after_ajax_export-' . $this->get_option_key(), $db_options, $sr_options, $result['status']);
	...
	function vafpress_verify_nonce()
	{
		$nonce  = $_POST['nonce'];
		...
		{
			$result['status']  = false;
			$result['message'] = esc_html__("Unverified Access", 'anchro');
		}
		return $result;
	}
	...
	function initial_db_setup()
	{
		...
		do_action('vafpress_option_before_db_init', $opt);

		// save to db
		$result = $this->save_and_reinit();

		// after db options db action hook
		do_action('vafpress_option_after_db_init', $opt, $result['status'], $this->get_option_key());
		do_action('vafpress_option_after_db_init-' . $this->get_option_key(), $opt, $result['status']);
	}
	...
	public function init_options_set()
	{
		...
		if( is_string($this->get_template()) and is_file($this->get_template()) )
			$template = include $this->get_template();//This method uses get_template_directory
		else if(is_array($this->get_template()))
			$template = $this->get_template();
		else
			throw new Exception(esc_html__( 'Invalid template supplied', 'anchro' ), 1);

		$parser = new VAFPRESS_Option_Parser();
		...
		{
			// setup utility menu
			$util_menu = new VAFPRESS_Option_Control_Group_Menu();
			$util_menu->set_title(esc_html__('Utility', 'anchro'));
			$util_menu->set_name('menu_util');
			$util_menu->set_icon('font-awesome:fa-ambulance');

			// setup restore default section
			$restore_section = new VAFPRESS_Option_Control_Group_Section();
			$restore_section->set_title(esc_html__('Restore Default', 'anchro'));
			$restore_section->set_name('section_restore');

			// setup restore button
			$restore_button = new VAFPRESS_Option_Control_Field_Restore();
			$restore_section->add_field($restore_button);

			// setup exim section
			$exim_section = new VAFPRESS_Option_Control_Group_Section();
			$exim_section->set_title(esc_html__('Export/Import', 'anchro'));
			$exim_section->set_name('section_exim');

			// setup exim field
			$exim_field = new VAFPRESS_Option_Control_Field_ImpExp();
	...
	{
		echo wp_kses_normalize_entities($this->get_options_set()->render());
	}
	...
	// @todo return `vafpress_option` like function
