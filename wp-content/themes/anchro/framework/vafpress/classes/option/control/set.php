...
class VAFPRESS_Option_Control_Set
{
	...
	public function render()
	{
		// Setup data
		$data = array('set' => $this);
		return VAFPRESS_View::instance()->load('option/set', $data);
	}
	...
	public function get_fields($include_section = false)
	{
		if(!function_exists('loop_controls'))
		{
			function loop_controls($menu, $include_section)
			{
				...
				{
					if( get_class($control) === 'VAFPRESS_Option_Control_Group_Section' )
					{
						if($include_section)
						{
						...
						{
							if( VAFPRESS_Util_Reflection::field_type_from_class(get_class($field)) != 'impexp' )
							{
					...
					{
						if( VAFPRESS_Util_Reflection::field_type_from_class(get_class($control)) != 'impexp' )
						{
							$fields[$control->get_name()] = $control;
						}
					}
	...
	public function process_binding()
	{
		...
		foreach ($fields as $field)
		{
			...
			if(!empty($bind) and is_null($val))
			{
				...
				if(VAFPRESS_Util_Reflection::is_multiselectable($field))
				{
					$result = (array) $result;
				}
				else
			...
			if($field instanceof VAFPRESS_Control_FieldMulti)
			{
	...
	public function normalize_values($opt_arr)
	{
		...
		foreach ($opt_arr as $key => $value)
		{
			...
			{
				$is_multi = VAFPRESS_Util_Reflection::is_multiselectable($fields[$key]);
	...
	public function save($option_key)
	{
		...
		do_action('vafpress_option_set_before_save', $opt);

		if(update_option($option_key, $opt))
		{
			$result['status']  = true;
			$result['code']    = self::SAVE_SUCCESS;
			$result['message'] = esc_html__('Saving successful', 'anchro');
		...
		else
		{
			...
			if($changed)
			{
				...
				$result['message'] = esc_html__('Saving failed', 'anchro');
			}
			else
			{
				$result['status']  = true;
				$result['code']    = self::SAVE_NOCHANGES;
				$result['message'] = esc_html__('No changes made', 'anchro');
			}
		}

		do_action('vafpress_option_set_after_save', $curr_opt, $result['status'], $option_key);
	...
	public function populate_values($opt, $force_update = false)
	{
		...
		foreach ( $fields as $field )
		{
			$is_multi = VAFPRESS_Util_Reflection::is_multiselectable($field);
